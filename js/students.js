...
let studentsName = tasksArray.map(element => element[0]);
let uniqueStudentsName = new Set(studentsName);

let allStudents = document.getElementById('allStudents');
let uniqueStudents = document.getElementById('uniqueStudents');
for (const std of studentsName) {
  allStudents.innerHTML += `<li>${std}</li>`
}
for (const std1 of uniqueStudentsName) {
  uniqueStudents.innerHTML += `<li>${std1}</li>`
}